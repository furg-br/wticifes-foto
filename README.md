...
Uma imagem só entra no feed se tiver simultaneamente `status=approved`, consentimento registrado, ausência de remoção/exclusão e prazo de publicação futuro. Cada download valida o estado e `token_version` no Postgres. Revogação e remoção incrementam essa versão, bloqueiam novas URLs imediatamente e solicitam exclusão física. URLs da vitrine duram cinco minutos e nunca expõem caminho do Blob, ID administrativo, participante, hashes ou auditoria. Em telas grandes, a vitrine masonry usa cinco colunas e preserva a variação natural de altura entre fotos verticais, horizontais e quadradas. Cada foto ocupa no máximo 45% da altura da tela, permitindo cerca de 12 fotos simultâneas sem alterar nem recortar o JPEG armazenado.
